...
function inputFormatPhoneInit(init_country, init_phone_number, lang) {
    ...
    for (c = 0; c < country_data.codes.length; c++) {
      ...
      if (code_item.patterns) {
        for (j = 0; j < code_item.patterns.length; j++) {
          var pattern = code_item.patterns[j],
            prefix = "",
            ph_prefix = country_code,
            new_pattern = "";
      ...
      if (code_item.prefixes) {
        for (j = 0; j < code_item.prefixes.length; j++) {
          var prefix = code_item.prefixes[j],
            ph_prefix = country_code + prefix;
          if (!prefix_map[ph_prefix]) {
            PrefixCountries.push([
              country_code,
              prefix,
              country_code + prefix,
              country_data,
  ...
  try {
    var compare = new Intl.Collator(lang || "en", { sensitivity: "base" })
      .compare;
  } catch (e) {
  ...
  function getPatternByPrefix(value) {
    for (var i = 0; i < PrefixPatterns.length; i++) {
      var pattern = PrefixPatterns[i][3],
        prefix = PrefixPatterns[i][2];
      if (value.indexOf(prefix) === 0) {
        return pattern;
      }
    }
  ...
  function onKeyDown(e) {
    if (
      e.target === phone_el &&
      (e.keyCode == Keys.LEFT || e.keyCode == Keys.BACKSPACE) &&
      ...
      phone_el.setSelectionRange(
        phone_el.selectionStart - 1,
        phone_el.selectionStart - 1
      );
    } else if (
      e.target === phone_el &&
  ...
  function onInput(e) {
    ...
    if (found_data) {
      isPrefixFull = true;
      var prefix = found_data.prefix,
        format = getPatternByPrefix(value) || "";
      ...
      var new_placeholder = new_value;
      country_label_el.innerHTML =
        country_label_el.getAttribute("data-placeholder");
  ...
  function updateSearchResults(query) {
    ...
    for (var i = 0; i < SortedCountriesList.length; i++) {
      ...
      if (
        ...
        (query[0] == "+" &&
          new RegExp(bre + cleanRE(query.substr(1)), "i").test(
            data.country_code
          ))
      ) {
        found = true;
        html +=
          '<div class="login_country_search_result" data-code="' +
          data.iso2 +
          '" data-prefix="' +
          data.country_code +
          '"><span dir="auto">' +
          data.lname +
          '</span><span dir="auto" class="prefix">+' +
          data.country_code +
          "</span></div>";
      }
    }
    if (!found) {
      ...
      html =
        '<div class="login_country_search_noresult">' +
        escapeHTML(noresult) +
        "</div>";
    }
